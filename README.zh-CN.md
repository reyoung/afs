...
只要所需 layer 在集群中的某个节点可访问，当前节点就可以通过网络读取并挂载文件系统。
...
- `afs_discovery_grpcd`
  - 服务发现、image 解析/缓存、节点状态管理（心跳）
  - 返回某个 image 或 layer 的 provider 列表

- `afs_layerstore_grpcd`
  - 只管理 layer 缓存
  - 提供 `EnsureLayers / HasLayer / StatLayer / ReadLayer / PruneCache` 等能力
  - 启动后向一个或多个 discovery 服务周期性上报
  - 支持缓存大小限制与 LRU 淘汰
    - 默认上限：`min(1TB, 缓存所在文件系统可用空间的60%)`
    - 可通过 `-cache-max-bytes` 配置
    - `EnsureLayers` 下载前会做磁盘空间预留（并发安全）
    - 如果请求的 layer 集超过缓存上限，直接失败
    - 超限时按访问时间做 LRU 淘汰，并触发心跳上报

- `afs_mount`
  - 以 discovery 作为控制面（`ResolveImage`、`FindImage`）
  - 以选定 layerstore 作为数据面（`EnsureLayers`、`ReadLayer`）
  - 先解析 image、选择完整 provider 或补齐缺失 layer，再执行挂载
...
推荐方式（Kubernetes + Helm 脚本）：

1. 创建 `helm/.env`，配置镜像仓库、命名空间、副本与资源
2. 执行 `./helm/start.sh`
3. 用 `kubectl -n <namespace> get deploy,ds,svc` 验证
4. 执行 `./helm/stop.sh` 停止
...
- 快速开始（中文）：`docs/getting_started.zh-CN.md`
